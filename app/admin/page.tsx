...
export default async function AdminProductsPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  try {
    const [response, categoriesResponse] = await Promise.all([
      getAdminProducts({ page, pageSize: 10, search, category, availability, status }),
      getAdminCategories(),
    ]);
    const products: ProductRow[] = response.data.map((product, index) => {
      const variants = (product as typeof product & { variants?: { label?: string; value?: string; stocks?: StrapiStock[] }[] }).variants ?? [];
      const hasDefaultVariant = variants.some((variant) => (variant as { label?: string; value?: string }).label === "Default" || (variant as { value?: string }).value === "General");
      const imageUrl = getStrapiMediaUrl(product.images?.[0]?.image?.url);
      const variantNames = variants.map((variant) => [variant.label, variant.value].filter(Boolean).join(" / ")).filter(Boolean);
      const stock = variants.reduce(
        (total, variant) => total + (variant.stocks?.reduce((sum, item) => sum + Number(item.quantity || 0), 0) ?? 0),
        0
      );
      return {
        ...
        variantLabel: variants.length === 0 ? "Sin variante" : hasDefaultVariant && variants.length === 1 ? "Default / General" : `${variants.length} variantes`,
        variantNames,
      };
    });
    ...
    const stats: ProductStat[] = [
      { label: "Total Productos", value: String(total), tone: "text-[#554246]" },
      { label: "En Stock", value: String(products.filter((product) => product.stock > 0 && product.active).length), tone: "text-emerald-700" },
      { label: "Stock Bajo", value: String(products.filter((product) => product.stock > 0 && product.stock <= 10).length), tone: "text-[#9E3659]" },
      { label: "Desactivados", value: String(products.filter((product) => !product.active).length), tone: "text-[#554246]" },
    ];
    const categories: ProductFilterOption[] = [
      { label: "Todas las Categorias", value: "all" },
      ...categoriesResponse.data.map((item) => ({ label: item.name, value: item.documentId })),
    ];
    const pagination: ProductPagination = { page, pageCount, total, pageSize: paginationMeta?.pageSize ?? 10 };
    return (
      <AdminProductsClient
        stats={stats}
        products={products}
    ...
    return (
      <AdminShell active="products" searchPlaceholder="Buscar productos...">
        <AdminDataError title="No se pudieron cargar productos desde Strapi" error={error} permissions={["Product: find/findOne/update", "Brand: find/findOne", "Category: find/findOne", "Variant-option: find/findOne/create", "Branch-stock: find/findOne", "Product-image: find/findOne", "Media Library: find/findOne"]} />
      </AdminShell>
    );
  }
}
